...
class MetricsExtractor:
    ...
    def get_metrics_config(self) -> Dict[str, str]:
        ...
        return {
            # Métricas de CPU
            'cpu_usage_total': 'rate(container_cpu_usage_seconds_total{filters}[5m])',
            'cpu_user': 'rate(container_cpu_user_seconds_total{filters}[5m])',
            'cpu_system': 'rate(container_cpu_system_seconds_total{filters}[5m])',
            'cpu_throttled_periods': 'rate(container_cpu_cfs_throttled_periods_total{filters}[5m])',
            'cpu_throttled_time': 'rate(container_cpu_cfs_throttled_seconds_total{filters}[5m])',
            
            # Métricas de Memória
            'memory_usage_bytes': 'container_memory_usage_bytes{filters}',
            'memory_working_set_bytes': 'container_memory_working_set_bytes{filters}',
            'memory_rss': 'container_memory_rss{filters}',
            'memory_cache': 'container_memory_cache{filters}',
            'memory_swap': 'container_memory_swap{filters}',
            'memory_max_usage': 'container_memory_max_usage_bytes{filters}',
            'memory_failures': 'rate(container_memory_failures_total{filters}[5m])',
            
            # Limites e Especificações
            'memory_limit': 'container_spec_memory_limit_bytes{filters}',
            'cpu_quota': 'container_spec_cpu_quota{filters}',
            'cpu_period': 'container_spec_cpu_period{filters}',
            
            # Métricas de Rede
            'network_rx_bytes': 'rate(container_network_receive_bytes_total{filters}[5m])',
            'network_tx_bytes': 'rate(container_network_transmit_bytes_total{filters}[5m])',
            'network_rx_errors': 'rate(container_network_receive_errors_total{filters}[5m])',
            'network_tx_errors': 'rate(container_network_transmit_errors_total{filters}[5m])',
            
            # Métricas de Disco
            'fs_usage_bytes': 'container_fs_usage_bytes{filters}',
            'fs_limit_bytes': 'container_fs_limit_bytes{filters}',
            'fs_reads': 'rate(container_fs_reads_bytes_total{filters}[5m])',
            'fs_writes': 'rate(container_fs_writes_bytes_total{filters}[5m])',
            
            # Processos
            'processes': 'container_processes{filters}',
            'threads': 'container_threads{filters}',
        }
    ...
    def extract_metrics(self, start_time: datetime, end_time: datetime, 
        ...
        for metric_name, metric_query in metrics_config.items():
            ...
            # Substitui {filters} pelos filtros reais
            query = metric_query.replace('{filters}', '{' + ','.join(filters) + '}')
...
if __name__ == "__main__":
    ...
    DURATION_MINUTES = 120  # Coleta últimos 60 minutos
    STEP = '30s'  # Medição a cada 30 segundos
    POD_FILTER = 'app-degradacao-57f8dfd878-gdvk5'  # Filtro opcional
