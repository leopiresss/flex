...
.PHONY: help build push deploy test clean status logs shell port-forward scale restart resources setup-local-registry cleanup-local-registry deploy-local deploy-no-push check-registry deploy-force deploy-emergency
...
help:
	...
	@echo "  deploy-force       Deploy with validation bypass (kubectl issues)"
	@echo "  deploy-emergency   Emergency deploy with inline manifests"
	...
	@echo "  deploy-force       Force deploy bypassing validation"
	@echo "  deploy-emergency   Emergency deploy with basic manifests"
	@echo ""
	@echo "Variables:"
	...
	@echo "  make deploy-force         # If kubectl has validation issues"
...
deploy-local:
	...
	@echo "Creating namespace if not exists..."
	kubectl create namespace $(NAMESPACE) --dry-run=client -o yaml | kubectl apply -f - || kubectl create namespace $(NAMESPACE) --validate=false || true
	...
	@sed 's/imagePullPolicy: IfNotPresent/imagePullPolicy: Never/g' deployment.yaml | kubectl apply -f - -n $(NAMESPACE) || \
	 sed 's/imagePullPolicy: IfNotPresent/imagePullPolicy: Never/g' deployment.yaml | kubectl apply -f - -n $(NAMESPACE) --validate=false
	kubectl apply -f service.yaml -n $(NAMESPACE) || kubectl apply -f service.yaml -n $(NAMESPACE) --validate=false
	kubectl apply -f ingress.yaml -n $(NAMESPACE) || kubectl apply -f ingress.yaml -n $(NAMESPACE) --validate=false || echo "⚠️ Ingress may not be supported"
	kubectl apply -f autoscaling.yaml -n $(NAMESPACE) || kubectl apply -f autoscaling.yaml -n $(NAMESPACE) --validate=false || echo "⚠️ HPA may not be supported"
	kubectl apply -f monitoring.yaml -n $(NAMESPACE) || kubectl apply -f monitoring.yaml -n $(NAMESPACE) --validate=false || echo "⚠️ Monitoring resources may require Prometheus operator"
	@echo "Waiting for deployment to be ready..."
	kubectl rollout status deployment/prime-server -n $(NAMESPACE) --timeout=300s
	@echo "✅ Local deployment completed"

# Deploy with validation bypass (for kubectl issues)
deploy-force:
	@echo "🚨 Force deployment with validation bypass..."
	...
	kubectl apply -f service.yaml -n $(NAMESPACE) --validate=false
	@echo "✅ Force deployment completed"

# Emergency deploy with script
deploy-emergency:
	@echo "🚨 Emergency deployment - using external script..."
	@if [ -f kubectl-emergency-fix.sh ]; then \
		chmod +x kubectl-emergency-fix.sh && ./kubectl-emergency-fix.sh; \
	else \
		echo "Emergency script not found. Using force deploy..."; \
		$(MAKE) deploy-force; \
	fi
